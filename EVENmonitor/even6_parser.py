...
class Substitution:
    ...
    def xml(self, template=None):
        ...
        if self._type == 0x1:
            ...
            return hex(int.from_bytes(value.data, "big"))
            ...
            return hex(int.from_bytes(value.data, "little"))
        elif self._type == 0x21:
            return value.template.xml()
        elif self._type == 0xf:
            return str(uuid.UUID(bytes_le=value.data))
        elif self._type == 0x14:    # Apparently raw hex values
            return hex(int.from_bytes(value.data, "little"))
        else:
            print("Unknown value type", hex(value.type), hex(self._type), value.data.hex())
            print(value.data.decode("utf-16le", errors="ignore"))
